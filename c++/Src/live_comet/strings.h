...
static inline std::string str(const char *s) {
	return std::string(s);
}
...
static inline std::string str(float v) {
	return str((double)v);
}
...
static inline std::string str_slice(const std::string &str, int start, int end) {
	if (start < 0) {
		start = (int)(str.size() + start);
	}
	int size;

...
public:
	...
	bool				AddClient(ISinkHttpRequest* client);
	const xtstring&		GetMoniker() const { return m_strMoniker; }
	Json::Value			GetStatus();

protected:
	virtual CodecType	PreferCodec() { return CodecType::Compressed; }
...
protected:
	...
	void				PostMediaData(const uint8_t* const lpData, const size_t nSize, const bool bKey);
	void				DoDiscard();

protected:
	void				write_avc_config(const uint8_t* config, int size);
	void				write_aac_config(const uint8_t* header, int size);
	void				write_avc_frame(const uint8_t* data, int size, uint32_t pts, bool key);
	void				write_aac_frame(const uint8_t* data, int size, uint32_t pts);
